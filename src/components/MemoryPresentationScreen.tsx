...
export function MemoryPresentationScreen({ presentation, onComplete, interrupted = false, previouslyStarted = false, onStart }: Props) {
  ...
  const durationMs = testMode ? 500 : presentation.durationSeconds * 1_000
  ...
  const visibleContent = isSequence ? [elapsed % slot < slot * .8 ? active.symbol : '＋'] : presentation.content
  ...
  return (
    <main className="app-shell encoding-screen">
      ...
      <section className="encoding-panel">
        ...
        {!started && !resumedMidway && <p className="memory-preview-note">点击开始后，你有{presentation.durationSeconds}秒可以看。时间到后，会自动进入题目。</p>}
        {resumedMidway ? <><p>这组材料刚才已经开始展示，不能再看一遍。我们会记下这次中断，报告中的记忆部分要多留意。</p><button className="primary-button" onClick={onComplete}>继续作答</button></> : !started ? (
          ...
          <>
            ...
            <div className={'memory-content ' + (isSequence ? 'sequence-content' : '')}>
              {isSequence && presentation.content.length > 1 ? <p>第{active.group + 1}组</p> : null}
              {presentation.visualIds ? presentation.visualIds.map(id => <div key={id}><MemoryGlyph id={id}/></div>) : visibleContent.map((line) => <div key={line}>{isSequence ? <SymbolGlyph symbol={line}/> : line}</div>)}
            </div>
          </>
        )}
